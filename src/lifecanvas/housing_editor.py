...
class HousingEditor(QGroupBox):
    """Simple housing plan: stay, sell and move, or keep and move."""
    ...
    def __init__(self, plan: ProjectPlan):
        super().__init__("住宅・住み替え")
        outer = QVBoxLayout(self)
        note = QLabel(
            "住み替えは大きな金額だけを入力します。売却諸費用や空室率などの細かな設定は使いません。"
        )
    ...
    def _keep_page(self) -> QWidget:
        ...
        form.addRow("新居の住宅費", self.new_home_monthly)
        form.addRow("旧居の手取り家賃", self.old_home_rent)
    ...
    def load(self, plan: ProjectPlan) -> None:
        ...
        index = self.mode.findData(house.move_mode)
    ...
    def apply_to(self, plan: ProjectPlan) -> None:
        ...
        house.move_mode = mode
        if mode == "none":
            house.move_offset = None
            return

        year = self.sell_year.int_value() if mode == "sell" else self.keep_year.int_value()

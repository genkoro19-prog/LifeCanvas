...
class GuidedInputPage(QScrollArea):
    """Guided baseline-plan input with live monthly cash-flow feedback."""
    ...
    def __init__(self, plan: ProjectPlan, parent: QWidget | None = None):
        ...
        self.setObjectName("guidedInputScroll")

        content = QWidget()
        content.setObjectName("guidedInputContent")
        layout = QVBoxLayout(content)
        layout.setContentsMargins(16, 12, 16, 22)
        layout.setSpacing(12)

        intro = QLabel(
            "今の収入・預金・生活費と、大きな予定だけで基準プランを作ります。"
            "細かな条件は『詳細設定』で後から変更できます。"
        )
        ...
        grid.addWidget(self._build_current_group(), 0, 0)
        grid.addWidget(self._build_flow_group(), 0, 1)
        grid.addWidget(self._build_family_group(), 1, 0)
        grid.addWidget(self._build_investment_group(), 1, 1)
        grid.setColumnStretch(0, 1)
        grid.setColumnStretch(1, 1)
        layout.addLayout(grid)

        review = QGroupBox("入力内容のチェック")
        review_layout = QVBoxLayout(review)
        review_note = QLabel(
            "二重計上、現在預金と最低現金の矛盾、出産時の投資増額、"
            "未確定の住み替えなどを計算前に確認します。"
        )
        review_note.setWordWrap(True)
        review_note.setStyleSheet("color:#5f6670;")
        self.review_list = QListWidget()
        self.review_list.setMinimumHeight(125)
        ...
        actions = QHBoxLayout()
        hint = QLabel(
            "入力を詳細設定へ反映し、基準・慎重・楽観の3プランを計算します。"
        )
        hint.setWordWrap(True)
        hint.setStyleSheet("color:#5f6670;")
    ...
    def _build_family_group(self) -> QGroupBox:
        ...
        for label, count in (
            ...
            self.child_count.addItem(label, count)
        ...
        move_note = QLabel(
            "未確定ならチェックを外してください。住み替えは比較シナリオで検討できます。"
        )
    ...
    def _update_enabled_fields(self) -> None:
        separate_personal = not self.includes_personal.isChecked()
        self.husband_personal.setEnabled(separate_personal)
        self.wife_personal.setEnabled(separate_personal)
    ...
    def _refresh_preview(self) -> None:
        ...
        self.husband_flow.setText(
            ...
            f"<br><b>毎月の預金増減 {self._signed_money(husband_change)}</b>"
        )
        self.wife_flow.setText(
            ...
            f"<br><b>毎月の預金増減 {self._signed_money(wife_change)}</b>"
        )
